...

{
	...
	// ================================================================
	// ESP Preview: mock player silhouette showing current ESP config.
	// Renders in its own ImGui window; toggle via Render::EspPreviewOpen.
	// ================================================================
	namespace {
		ImU32 PrvToU32(ImColor c) {
		...
		struct PrvVec2 { float x, y; };

		enum PrvBone {
			PRV_HEAD = 0, PRV_NECK, PRV_CHEST, PRV_SPINE2, PRV_SPINE1, PRV_PELVIS,
			PRV_SHOULDER_L, PRV_ELBOW_L, PRV_HAND_L,
			PRV_SHOULDER_R, PRV_ELBOW_R, PRV_HAND_R,
			PRV_HIP_L, PRV_KNEE_L, PRV_FOOT_HEEL_L, PRV_FOOT_TOES_L,
			PRV_HIP_R, PRV_KNEE_R, PRV_FOOT_HEEL_R, PRV_FOOT_TOES_R,
		};

		PrvVec2 PrvBonePos(int boneId, float cx, float top, float scale) {
			float s = scale;
			switch (boneId) {
			case PRV_HEAD:        return { cx,          top + 10*s  };
			case PRV_NECK:        return { cx,          top + 22*s  };
			case PRV_CHEST:       return { cx,          top + 38*s  };
			case PRV_SPINE2:      return { cx,          top + 54*s  };
			case PRV_SPINE1:      return { cx,          top + 70*s  };
			case PRV_PELVIS:      return { cx,          top + 90*s  };
			case PRV_SHOULDER_L:  return { cx + 18*s,   top + 26*s  };
			case PRV_ELBOW_L:     return { cx + 32*s,   top + 52*s  };
			case PRV_HAND_L:      return { cx + 34*s,   top + 70*s  };
			case PRV_SHOULDER_R:  return { cx - 18*s,   top + 26*s  };
			case PRV_ELBOW_R:     return { cx - 32*s,   top + 52*s  };
			case PRV_HAND_R:      return { cx - 34*s,   top + 70*s  };
			case PRV_HIP_L:       return { cx + 10*s,   top + 95*s  };
			case PRV_KNEE_L:      return { cx + 13*s,   top + 125*s };
			case PRV_FOOT_HEEL_L: return { cx + 14*s,   top + 146*s };
			case PRV_FOOT_TOES_L: return { cx + 16*s,   top + 154*s };
			case PRV_HIP_R:       return { cx - 10*s,   top + 95*s  };
			case PRV_KNEE_R:      return { cx - 13*s,   top + 125*s };
			case PRV_FOOT_HEEL_R: return { cx - 14*s,   top + 146*s };
			case PRV_FOOT_TOES_R: return { cx - 16*s,   top + 154*s };
			default:              return { cx,          top + 50*s  };
			}
		}

		struct PrvBonePair { int from, to; };
		const PrvBonePair kPrvPairs[] = {
			{PRV_PELVIS,      PRV_SPINE1},
			{PRV_SPINE1,      PRV_SPINE2},
			{PRV_SPINE2,      PRV_CHEST},
			{PRV_CHEST,       PRV_NECK},
			{PRV_NECK,        PRV_HEAD},
			{PRV_NECK,        PRV_SHOULDER_L},
			{PRV_SHOULDER_L,  PRV_ELBOW_L},
			{PRV_ELBOW_L,     PRV_HAND_L},
			{PRV_NECK,        PRV_SHOULDER_R},
			{PRV_SHOULDER_R,  PRV_ELBOW_R},
			{PRV_ELBOW_R,     PRV_HAND_R},
			{PRV_PELVIS,      PRV_HIP_L},
			{PRV_HIP_L,       PRV_KNEE_L},
			{PRV_KNEE_L,      PRV_FOOT_HEEL_L},
			{PRV_FOOT_HEEL_L, PRV_FOOT_TOES_L},
			{PRV_PELVIS,      PRV_HIP_R},
			{PRV_HIP_R,       PRV_KNEE_R},
			{PRV_KNEE_R,      PRV_FOOT_HEEL_R},
			{PRV_FOOT_HEEL_R, PRV_FOOT_TOES_R},
		};

		void PrvDrawPlayer(ImDrawList* dl, float cx, float boxTop,
		                   float boxW, float boxH, float boneScale,
		                   ImU32 entityCol, bool showBottomLabels,
		                   float areaBottom, bool useVisColors) {
			...
			// Snapline / trace line (ShowLineToEnemy or ShowEyeRay)
			if ((MenuConfig::ShowLineToEnemy || MenuConfig::ShowEyeRay) && showBottomLabels) {
				float fromY = (MenuConfig::LineToEnemyOrigin == 0) ? (boxTop - 30) : areaBottom;
				dl->AddLine(ImVec2(cx, fromY), ImVec2(cx, boxTop + boxH),
				            IM_COL32(0, 0, 0, 180), 2.5f);
				dl->AddLine(ImVec2(cx, fromY), ImVec2(cx, boxTop + boxH),
				            PrvToU32(MenuConfig::LineToEnemyColor), 1.0f);
			}

			// Corner box
			if (MenuConfig::ShowBoxESP)
				PrvCornerBox(dl, boxLeft, boxTop, boxW, boxH,
				             entityCol, boxH * 0.22f,
				             std::clamp(MenuConfig::BoxThickness, 0.5f, 4.0f));

			// Side bars (health + armor)
			const float healthBarLeft = boxLeft - sideBarW - sideBarGap;
			const float armorBarLeft = MenuConfig::ShowHealthBar
			...
			if (MenuConfig::ShowArmorBar) {
				PrvSideBar(dl, armorBarLeft, boxTop, boxH, sideBarW, apFrac,
				           PrvToU32(MenuConfig::ArmorBarColor));
			}
			if (MenuConfig::ShowHealthBar) {
				ImU32 hpCol = HealthColorFromRatio(mockHp);
				PrvSideBar(dl, healthBarLeft, boxTop, boxH, sideBarW, hpFrac, hpCol);
			}

			// Bar value labels
			if (MenuConfig::ShowHealthText && MenuConfig::ShowHealthBar) {
				char buf[8];
				snprintf(buf, sizeof(buf), "%d", mockHp);
				ImVec2 ts = ImGui::CalcTextSize(buf);
				float lx = healthBarLeft + sideBarW * 0.5f - ts.x * 0.5f;
				float ly = boxTop - ts.y - 3.0f;
				DrawTextShadow(dl, nullptr, 0.f, ImVec2(lx, ly),
				               IM_COL32(255, 255, 255, 255), buf);
			}
			if (MenuConfig::ShowArmorText && MenuConfig::ShowArmorBar) {
				char buf[8];
				snprintf(buf, sizeof(buf), "%d", mockArmor);
				ImVec2 ts = ImGui::CalcTextSize(buf);
				float lx = armorBarLeft + sideBarW * 0.5f - ts.x * 0.5f;
				float ly = boxTop - ts.y - 3.0f;
				DrawTextShadow(dl, nullptr, 0.f, ImVec2(lx, ly),
				               IM_COL32(225, 245, 255, 255), buf);
			}

			// Skeleton (22 joints, 19 bone pairs)
			if (MenuConfig::ShowBoneESP) {
				ImU32 skelCol = (MenuConfig::VisibilityColoring && useVisColors)
				                ? entityCol : PrvToU32(MenuConfig::BoneColor);
				const float skelThick = std::clamp(MenuConfig::BoneThickness, 0.5f, 4.0f);
				const float skelOutline = skelThick + 1.4f;
				for (const auto& p : kPrvPairs) {
					PrvVec2 a = PrvBonePos(p.from, cx, boxTop, boneScale);
					PrvVec2 b = PrvBonePos(p.to, cx, boxTop, boneScale);
					dl->AddLine(ImVec2(a.x, a.y), ImVec2(b.x, b.y),
					            IM_COL32(0, 0, 0, 200), skelOutline);
				}
				for (const auto& p : kPrvPairs) {
					PrvVec2 a = PrvBonePos(p.from, cx, boxTop, boneScale);
					PrvVec2 b = PrvBonePos(p.to, cx, boxTop, boneScale);
					dl->AddLine(ImVec2(a.x, a.y), ImVec2(b.x, b.y), skelCol, skelThick);
				}
			}

			// Head dot
			if (MenuConfig::ShowHeadDot) {
				PrvVec2 head = PrvBonePos(PRV_HEAD, cx, boxTop, boneScale);
				float r = std::max(1.5f, MenuConfig::HeadDotSize);
				dl->AddCircleFilled(ImVec2(head.x, head.y), r,
				                    PrvToU32(MenuConfig::HeadDotColor), 12);
				dl->AddCircle(ImVec2(head.x, head.y), r,
			...
			if (MenuConfig::ShowPlayerName) {
				...
				ImFont* font = ImGui::GetFont();
				float fs = MenuConfig::NameFontSize > 0 ? MenuConfig::NameFontSize : ImGui::GetFontSize();
				ImVec2 ts = font ? font->CalcTextSizeA(fs, FLT_MAX, 0.0f, name)
				                 : ImGui::CalcTextSize(name);
				DrawTextShadow(dl, font, fs,
				               ImVec2(cx - ts.x * 0.5f, boxTop - ts.y - 4),
				               PrvToU32(MenuConfig::NameColor), name);
			}

			// Weapon + ammo (below box)
			float bottomY = boxTop + boxH + 4;
			if (MenuConfig::ShowWeaponESP) {
				// Weapon icon (uses "W" placeholder glyph)
				if (MenuConfig::ShowWeaponIcon) {
					const char* icon = "W";
					ImFont* iconFont = Gui.weaponIconFont;
					float iconSize = MenuConfig::WeaponIconFontSize;
					if (iconFont) {
						ImVec2 ts = iconFont->CalcTextSizeA(iconSize, FLT_MAX, 0.0f, icon);
						DrawTextShadow(dl, iconFont, iconSize,
						               ImVec2(cx - ts.x * 0.5f, bottomY),
						               PrvToU32(MenuConfig::WeaponIconColor), icon);
						bottomY += ts.y + 1;
					} else {
						ImVec2 ts = ImGui::CalcTextSize(icon);
						DrawTextShadow(dl, nullptr, 0.f,
						               ImVec2(cx - ts.x * 0.5f, bottomY),
						               PrvToU32(MenuConfig::WeaponIconColor), icon);
						bottomY += ts.y + 1;
					}
				...
				// Weapon text
				{
					...
					ImFont* font = ImGui::GetFont();
					float fs = MenuConfig::WeaponFontSize > 0.0f ? MenuConfig::WeaponFontSize : ImGui::GetFontSize();
					ImVec2 ts = font ? font->CalcTextSizeA(fs, FLT_MAX, 0.0f, weapon)
					                 : ImGui::CalcTextSize(weapon);
					DrawTextShadow(dl, font, fs,
					               ImVec2(cx - ts.x * 0.5f, bottomY),
					               PrvToU32(MenuConfig::WeaponColor), weapon);
					bottomY += ts.y + 2;
				}
			}
			if (MenuConfig::ShowWeaponAmmo) {
				...
				ImFont* font = ImGui::GetFont();
				float fs = MenuConfig::WeaponAmmoFontSize > 0.0f ? MenuConfig::WeaponAmmoFontSize : ImGui::GetFontSize();
				ImVec2 ts = font ? font->CalcTextSizeA(fs, FLT_MAX, 0.0f, ammo)
				                 : ImGui::CalcTextSize(ammo);
				DrawTextShadow(dl, font, fs,
				               ImVec2(cx - ts.x * 0.5f, bottomY),
				               PrvToU32(MenuConfig::WeaponAmmoColor), ammo);
			}

			// Player flags + distance (right side of box)
			if (MenuConfig::ShowPlayerFlags || MenuConfig::ShowDistance) {
				float flagY = boxTop;
				float flagX = boxLeft + boxW + 6;
				float fs = MenuConfig::FlagFontSize > 0 ? MenuConfig::FlagFontSize : ImGui::GetFontSize();

				if (MenuConfig::ShowPlayerFlags) {
					if (MenuConfig::FlagScopedEnabled) {
						DrawTextShadow(dl, nullptr, 0.f, ImVec2(flagX, flagY),
						               PrvToU32(MenuConfig::FlagScopedColor), "Scoped");
						flagY += fs + 1;
					}
					if (MenuConfig::FlagMoneyEnabled) {
						DrawTextShadow(dl, nullptr, 0.f, ImVec2(flagX, flagY),
						               PrvToU32(MenuConfig::FlagMoneyColor), "$4200");
						flagY += fs + 1;
					}
				}
				if (MenuConfig::ShowDistance) {
					DrawTextShadow(dl, nullptr, 0.f, ImVec2(flagX, flagY),
					               PrvToU32(MenuConfig::DistanceColor), "42m");
				}
			}
		}
	} // anonymous namespace
	...
	void RenderEspPreview() {
		if (!EspPreviewOpen) return;

		const bool triMode = MenuConfig::VisibilityColoring;
		...
		if (!ImGui::Begin("ESP Preview", &EspPreviewOpen,
		...
		if (triMode) {
			// Three-column mode: Default / Visible / Hidden
			...
			ImU32 defCol = PrvToU32(MenuConfig::BoxColor);
			PrvDrawPlayer(dl, cx1, boxTop, boxW, boxH, boneScale,
			              defCol, true, winPos.y + ph - 22, false);

			// Visible (VisibleColor)
			float cx2 = winPos.x + thirdW * 1.5f;
			ImU32 visCol = PrvToU32(MenuConfig::VisibleColor);
			PrvDrawPlayer(dl, cx2, boxTop, boxW, boxH, boneScale,
			              visCol, true, winPos.y + ph - 22, true);

			// Hidden (HiddenColor)
			float cx3 = winPos.x + thirdW * 2.5f;
			ImU32 hidCol = PrvToU32(MenuConfig::HiddenColor);
			PrvDrawPlayer(dl, cx3, boxTop, boxW, boxH, boneScale,
			              hidCol, true, winPos.y + ph - 22, true);
			...
			DrawTextShadow(dl, nullptr, 0.f,
			               ImVec2(cx1 - dlSize.x * 0.5f, labelY),
			               IM_COL32(180, 180, 180, 255), defLabel);
			DrawTextShadow(dl, nullptr, 0.f,
			               ImVec2(cx2 - vlSize.x * 0.5f, labelY), visCol, visLabel);
			DrawTextShadow(dl, nullptr, 0.f,
			               ImVec2(cx3 - hlSize.x * 0.5f, labelY), hidCol, hidLabel);
		} else {
			// Single mode: one player with BoxColor
			...
			ImU32 entityCol = PrvToU32(MenuConfig::BoxColor);
			PrvDrawPlayer(dl, cx, boxTop, boxW, boxH, boneScale,
			              entityCol, true, winPos.y + ph, false);
		}
